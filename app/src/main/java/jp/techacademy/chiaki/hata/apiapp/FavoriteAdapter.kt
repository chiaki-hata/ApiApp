...
class FavoriteAdapter(private val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private fun updateFavoriteItemViewHolder(holder: FavoriteItemViewHolder,position: Int) {
        ...
        holder.apply {
            rootView.apply {
                setBackgroundColor(ContextCompat.getColor(context, if (position % 2 == 0) android.R.color.white
                else android.R.color.darker_gray)
                )
                setOnClickListener {
                    var couponUrls = CouponUrls(data.url, data.url)

                    var shop = Shop(couponUrls, data.id, data.imageUrl, data.name, data.address)
